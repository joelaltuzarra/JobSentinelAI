...
class GeminiConnector(BaseLLMConnector):
    ...
    def extract_skills_from_cv(self, cv_text: str) -> list:
        ...
        habilidades técnicas y blandas mencionadas (lenguajes, frameworks, metodologías).
...
class DeepSeekConnector(BaseLLMConnector):
    ...
    def _call_api(self, prompt: str) -> list:
        try:
            response = self.client.chat.completions.create(
                model="deepseek-chat",
                messages=[
                    {"role": "system", "content": "You are a helpful assistant designed to output strict JSON holding a list of skills."},
    ...
    def extract_skills_from_cv(self, cv_text: str) -> list:
        prompt = f"""Extrae las habilidades técnicas y blandas del CV.
...
class OllamaConnector(BaseLLMConnector):
    ...
    def extract_skills_from_cv(self, cv_text: str) -> list:
        prompt = f"""Extrae TODAS las habilidades técnicas y blandas del CV. 
        Ignora los nombres, fechas y empresas.
        Devuelve estrictamente JSON: {{"skills": ["skill1", "skill2", "skill3"]}}.\nCV:\n{cv_text[:3000]}"""
        return self._call_api(prompt)

    def extract_skills_from_job(self, job_description: str) -> list:
        prompt = f"""Extrae TODAS las habilidades técnicas y blandas requeridas para el cargo.
        - Sé conciso (ej. usa "aws" en lugar de "conocimiento avanzado en aws").
        - Trata las tecnologías empaquetadas como un solo item.
        Devuelve estrictamente JSON: {{"skills": ["skill1", "skill2", "skill3"]}}.\nCargo:\n{job_description[:3000]}"""
        return self._call_api(prompt)
...
def get_llm_connector(provider: str) -> BaseLLMConnector:
    """
    ...
    if provider == "gemini":
        ...
        return OllamaConnector(model_name="llama3.1:8b")  # Puedes cambiar el modelo si tienes otro disponible
    else:
